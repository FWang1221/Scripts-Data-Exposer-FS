...
function runDebugFile()
    pcall(loadfile("scriptExposerAssets\\hks\\debug.lua"))
end

Update = createDetour(Update, runDebugFile)

function breakPointFunc()
    addCondiBreakpoint(env(1108, ACTION_ARM_EAVESDROP) > 5000)
end

Update = createDetour(Update, breakPointFunc)

os.clock = function() return getOSClockLua() end


function getPtr(...)
    return traversePointerChain(10000, 0, ...)
end

function setPtr(...)
    return writePointerChain(10000, 0, ...)
end

function setWorldSpeed(speed)
    setTimeStepSize((1 / 60) * speed)
end
...
dynamicBackstepFuncActive = json.decode(secondEnvGetGlobalJSON("dynamicBackstepFuncActive"))
function enableOrDisableDynamicParamsBackstep()
    if dynamicBackstepFuncActive == "true" then
        dynamicBackstepFuncActive = "false"
    else
        dynamicBackstepFuncActive = "true"
    end
    secondEnvRunCode("dynamicBackstepFuncActive = \"" .. tostring(dynamicBackstepFuncActive) .. "\"")
    exposePrint("Dynamic backstep is now " .. tostring(dynamicBackstepFuncActive))
end
function dynamicBackstep()
    if env(1118) == TRUE and dynamicBackstepFuncActive then
        paramUtils.setParamRowAttributeWithBackup("ChrPhysicsVelocityChangeParam", 100, "horizontalVelocityChange", 15)
        paramUtils.setParamRowAttributeWithBackup("ChrPhysicsVelocityChangeParam", 100, "horizontalVelocityAngle", 180)
        paramUtils.setParamRowAttributeWithBackup("ChrPhysicsVelocityChangeParam", 100, "verticalVelocityChange", 0)
    else
        paramUtils.setParamRowAttributeWithBackup("ChrPhysicsVelocityChangeParam", 100, "horizontalVelocityChange", 0)
        paramUtils.setParamRowAttributeWithBackup("ChrPhysicsVelocityChangeParam", 100, "horizontalVelocityAngle", 0)
        paramUtils.setParamRowAttributeWithBackup("ChrPhysicsVelocityChangeParam", 100, "verticalVelocityChange", 9.64)
    end
end

addDebugFunc("Enable Vertical Jump Backstep", "Enables/Disables when locked on, vertical jump (jump without directional input) gives a quick backstep instead.", {"backstep"}, function() enableOrDisableDynamicParamsBackstep() end)

function adrenalineMode()
    local worldSpeed = askQuestion("What should the world speed be?")
    local playerSpeed = askQuestion("What should the player speed be?")
    setWorldSpeed(worldSpeed)
    playerData.setAnimSpeed(playerSpeed)
end

addDebugFunc("Adrenaline Mode (Set World/Player Speed)", "Sets world and player speed relative to it. For example: World speed at 1.25 and Player speed at 0.8 means that the player moves like normal (perceived) but the world is 25 percent faster.", {"speed", "adrenaline", "adrenaline mode"}, function() adrenalineMode() end)


function CameraChangeMode()
    local fovChange = askQuestion("What should the FOV change be? (1 for no change, 2 for double, 0.5 for half, etc.)")
    local heightChange = askQuestion("What should the height change be? (1 for no change, 2 for double, 0.5 for half, etc.)")
    local distanceChange = askQuestion("What should the distance change be? (1 for no change, 2 for double, 0.5 for half, etc.)")
    local cameraRows = paramUtils.getParamRows("LockCamParam")
    for k, v in pairs(cameraRows) do
        local rowNum = v.id
        paramUtils.setParamRowAttributeWithBackup("LockCamParam", rowNum, "camFovY", paramUtils.getParamRowAttribute("LockCamParam", rowNum, "camFovY") * fovChange)
        paramUtils.setParamRowAttributeWithBackup("LockCamParam", rowNum, "chrOrgOffset_Y", paramUtils.getParamRowAttribute("LockCamParam", rowNum, "chrOrgOffset_Y") * heightChange)
        paramUtils.setParamRowAttributeWithBackup("LockCamParam", rowNum, "camDistTarget", paramUtils.getParamRowAttribute("LockCamParam", rowNum, "camDistTarget") * distanceChange)
        
    end
end

addDebugFunc("Camera Change Mode", "Multiplies all camera FOVs, heights, and distances by a set amount.", {"cam", "camera", "change camera"}, function() CameraChangeMode() end)


function InvisibleEnemiesMode()
    local enemyRows = paramUtils.getParamRows("SpEffectParam")


    local hallucinationSpEffectVFX = 8
    local phantomRow = 1

    for k, v in pairs(enemyRows) do
        local rowNum = v.id
        if rowNum >= 7000 and rowNum <= 8000 then
            paramUtils.setParamRowAttributeWithBackup("SpEffectParam", rowNum, "vfxId6", hallucinationSpEffectVFX)
        end
    end

    paramUtils.setParamRowAttributeWithBackup("SpEffectVfxParam", hallucinationSpEffectVFX, "fadeInRateCamouflage", 100)
    paramUtils.setParamRowAttributeWithBackup("SpEffectVfxParam", hallucinationSpEffectVFX, "fadeOutRateCamouflage", 100)
    paramUtils.setParamRowAttributeWithBackup("SpEffectVfxParam", hallucinationSpEffectVFX, "phantomParamOverwriteId", 1)
    paramUtils.setParamRowAttributeWithBackup("SpEffectVfxParam", hallucinationSpEffectVFX, "phantomParamOverwriteType", 2)
    paramUtils.setParamRowAttributeWithBackup("PhantomParam", phantomRow, "alpha", 0)
end

addDebugFunc("Invisible Enemies Mode", "Makes all enemies invisible", {"invisible", "invisible enemies","invisible enemies mode"}, function() InvisibleEnemiesMode() end)
...
paramsDataByName = {}
local thing1, thing2 = pcall(loadfile("scriptExposerAssets\\hks\\eladiduParams.lua"))
...
if not thing1 then
    ...
    if hasStartedStr == json.encode("false") then
        ...
        for i, v in pairs(paramsDataByName) do
            if v.rows ~= nil then
                file = io.open("scriptExposerAssets\\temp\\" .. i .. ".lua", "w")
        ...
        local function restoreBackupParams()
            local paramBackups = json.decode(secondEnvGetGlobalJSON("paramBackups"))
            for i, v in pairs(paramBackups) do
                for j, k in pairs(v) do
                    for l, m in pairs(k) do
                        paramUtils.setParamRowAttribute(i, j, l, m)
                    end
                end
            end
        end
        ...
        for i, v in pairs(paramsLoaded) do
            print("Loading : " .. "scriptExposerAssets\\temp\\" .. v .. ".lua")
            local thing3, thing4 = pcall(loadfile("scriptExposerAssets\\temp\\" .. v .. ".lua"))
            if not thing3 then
        ...
        restoreBackupParams()
        print("loaded backed up params")
    end
    --print(json.encode(paramUtils.getParamRows("EquipParamGoods")))
end

function restoreBackupParams()
    local paramBackups = json.decode(secondEnvGetGlobalJSON("paramBackups"))
    for i, v in pairs(paramBackups) do
        for j, k in pairs(v) do
            for l, m in pairs(k) do
