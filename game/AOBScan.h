...
static std::map<std::string, void*> registeredAddresses;
static std::map<std::string, int> staticOffsets;
...
uintptr_t hexStringToUintptr(const std::string& hexStr) {
  return static_cast<uintptr_t>(std::stoull(hexStr, nullptr, 16));
}


std::string findGameFromIni(const std::string& iniPath) {
  // Create a SimpleIni object
  CSimpleIniA ini;
  ini.SetUnicode();
  ini.LoadFile(iniPath.c_str());

  const char* name = ini.GetValue("General", "Game", "");

  return name;
}

std::string aobFilePath() {
  std::string path = "scriptExposerAssets";
  std::string game = findGameFromIni(path + "/Settings.ini");
  path += "/" + game + "/AOBs.ini";
  return path;
}
void LoadFromIniAndRegister(const std::string& sectionName, void** storeTo) {

  const std::string iniPath = aobFilePath();
  Logger::log("Loading AOB: " + sectionName + " from: " + iniPath);
  ...
  const char* name = ini.GetValue(sectionName.c_str(), "name", "");
  const char* staticOffset = ini.GetValue(sectionName.c_str(), "staticOffset", "");
  const char* aob = ini.GetValue(sectionName.c_str(), "aob", "");
  int aobOffset = std::stoi(ini.GetValue(sectionName.c_str(), "aobOffset", "0"));

  if (name && staticOffset && aob) {
    uintptr_t offset = hexStringToUintptr(staticOffset);  // Convert hex offset

    RegisterAddress(sectionName.c_str(), (void**)&storeTo, AOBScanCode(aob, aobOffset));

    staticOffsets[sectionName.c_str()] = offset;
  }
  else {
    std::cerr << "Error reading section: " << sectionName.c_str() << std::endl;
  }
}
extern void ScanAndAssignAddresses() 
{
  ...
  LoadFromIniAndRegister("SoloParamRepository", SoloParamRepository);
  LoadFromIniAndRegister("WorldChrMan", WorldChrMan);

  //misc
  LoadFromIniAndRegister("WorldChrManDbg", WorldChrManDbg);
  LoadFromIniAndRegister("WorldAiManager", WorldAiManager);
  LoadFromIniAndRegister("GameMan", GameMan);
  LoadFromIniAndRegister("FieldArea", FieldArea);
  LoadFromIniAndRegister("GameData", GameData);
  LoadFromIniAndRegister("EventMan", EventMan);
  LoadFromIniAndRegister("EventFlagMan", EventFlagMan);
  LoadFromIniAndRegister("LockTgtMan", LockTgtMan);
  LoadFromIniAndRegister("DamageManagement", DamageManagement);
  LoadFromIniAndRegister("MapItemMan", MapItemMan);
  LoadFromIniAndRegister("Dlc", Dlc);
  LoadFromIniAndRegister("DebugFlags", DebugFlags);
  LoadFromIniAndRegister("DebugMenu", DebugMenu);
  LoadFromIniAndRegister("RenderFlags", RenderFlags);
  LoadFromIniAndRegister("TargetingDrawFlags", TargetingDrawFlags);
  LoadFromIniAndRegister("RendMan", RendMan);
